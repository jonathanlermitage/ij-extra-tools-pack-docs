...
Go to `Help > Edit Custom Properties...` then add `idea.is.internal=true`. Restart your IDE. You can now use the UI Inspector with this combination: `Ctrl` + `Alt + clicking on an element in the IDE. This will open a dialog showing information about the element. Click `icon` to get the icon path.
